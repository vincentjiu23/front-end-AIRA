...
const UploadPrognosis: React.FC = () => {
  ...
  const handleSubmit = async () => {
    if (!selectedCancer || !geneFile) return;

    const slug = selectedCancer.toLowerCase().replace(/\s+/g, '-');
    const url = `${baseUrl}/cancers/${slug}/predict?ai_feature=gene&feature_key=gene`;
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      <h2 className="text-xl font-bold mb-4">Upload Prognosis (Gene Only)</h2>
